...
def render(image)
  min_x, min_y, max_x, max_y = dimensions(image)

  padding = 5
  (min_x - padding).upto(max_x + padding) do |x|
    (min_y - padding).upto(max_y + padding) do |y|
      print image[[x, y]] ? "\u{2592}" : ' '
    end
...
puts File
       ...
       .then { |algo, image| 2.times { |i| image = apply_algo(algo, image, i) }; image }
